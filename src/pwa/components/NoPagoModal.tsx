    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white border-t sm:border border-slate-200 rounded-t-3xl sm:rounded-2xl w-full max-w-md p-5 pb-8 sm:pb-5 space-y-4 shadow-2xl">
          ...
          {opciones.map((op) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <div className={`font-bold text-sm ${isSelected ? 'text-amber-950' : 'text-slate-800'}`}>{op.label}</div>
                  <div className={`text-xs ${isSelected ? 'text-amber-800' : 'text-slate-500'}`}>{op.desc}</div>
                </div>
              </button>
            );
          })}
        ...
        <div>
          ...
          <input
            type="text"
            placeholder="Ej. Dejó dicho la empleada que pasa a las 18 hs"
            value={observacion}
            onChange={(e) => setObservacion(e.target.value)}
            className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:bg-white transition"
          />
        </div>
        ...
        <button
          ...
          className="w-full h-13 py-3 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl text-sm transition shadow-md shadow-amber-600/20 flex items-center justify-center gap-2"
        >
